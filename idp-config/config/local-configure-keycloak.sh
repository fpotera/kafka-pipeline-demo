...
export KEYCLOAK_URL=http://idp:8080
...
function configureKeycloak() {
    ...
    GRANT_TYPE=password
    CLIENT_ID=admin-cli
    TEST_REALM=/config/test-realm.json
    ...
    result=$(curl -d @"$TEST_REALM" -H "Content-Type: application/json" -H "Authorization: bearer $access_token" "$KEYCLOAK_URL/admin/realms")
